 ...
 * Part 1 uses the FP_ADD outside the CORDIC architecture.
 *
*/
#include <stdlib.h>
...
float sumVector(float x[])
{
	...
	float sum = 0;
	for( ; i < N; i++)
		sum = ALT_CI_FP_ADD_0(sum, ALT_CI_FP_ADD_0(ALT_CI_FP_MULT_0(0.5,x[i]),ALT_CI_FP_MULT_0(x[i],ALT_CI_FP_MULT_0(x[i],ALT_CI_CORDIC_0(x[i])))));
	return sum;
}
